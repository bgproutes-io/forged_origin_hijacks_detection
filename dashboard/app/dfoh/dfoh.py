...
def _parse_dt_local(s: str) -> Optional[datetime]:
   """Parse datetime-local string -> datetime. Return None if blank."""
   if not s:
       return None
...
def _parse_row_dt(s: str) -> Optional[datetime]:
   # Row strings like "YYYY-MM-DD HH:MM:SS"
   try:
...
def _contains_any(haystack_list: List[str], needle_csv: str) -> bool:
   """
   True if any value from comma-separated 'needle_csv' appears (substring match)
   in any element of 'haystack_list'. Empty needle_csv -> True (no filter).
   """
   if not needle_csv.strip():
       return True
   needles = [x.strip() for x in needle_csv.split(",") if x.strip()]
   if not needles:
       return True
   H = " ".join(haystack_list).lower()
...
def index() -> rx.Component:
   return rx.fragment(
       ...
       rx.box(
           ...
           padding_y="2rem",
           style={"maxWidth": "1400px", "margin": "0 auto"},  # ✅ applies only to content
       ),
   )
