    ...
    for t in table:
        for i in range(0, len(t)):
            ...
            if float(t[i][0])+float(t[i][1]) == 0:
                ...
                t[i] = float("%.5f" %  (float(t[i][0])/(float(t[i][0])+float(t[i][1]))))
...
def plot_heatmap(date, db_dir, edges_file, outfile, title=None):
    graph_file = "{}/full_topology/{}_full.txt".format(db_dir, date)
...
def plot_heatmap_rate(date, db_dir, edges_file1, edges_file2, outfile):
    # Full graph

    thresholds = [0, 10, 50, 100, 500, 1000, 1500, 3000, 5000, 100000]
    table = compute_table_rate(date, db_dir, edges_file1, edges_file2)
...
#plot_heatmap("../../new_sampling/graph_complete.txt", "../../new_sampling/positives_links_aspaths.txt", "topo_simu.pdf")
    ...
    for feats in features:
        plot_heatmap_rate(date, db_dir, 
                            "../main/tmp/tp_{}.txt".format("_".join(sorted(feats))), 
                            "../main/tmp/fn_{}.txt".format("_".join(sorted(feats))), 
                            "pdfs/TPR_{}.pdf".format("_".join(sorted(feats))))
        
        plt.clf()

        plot_heatmap_rate(date, db_dir, 
                            "../main/tmp/fp_{}.txt".format("_".join(sorted(feats))), 
                            "../main/tmp/tn_{}.txt".format("_".join(sorted(feats))), 
                            "pdfs/FPR_{}.pdf".format("_".join(sorted(feats))))

        plt.clf()

    ps.load_dataset("2022-08-31", db_dir)
